...
export const addPipesAtLocations = (x, world, entities) => {
    let [pipe1Height, pipe2Height] = generatePipes()

    let pipeTopWidth = Constants.PIPE_WIDTH + 20
    let pipeTopHeight = (pipeTopWidth / 224) * 102
...
const Physics = (entities, { touches, time, dispatch }) => {
    ...
    touches.filter(t => t.type === 'press').forEach(t => {
        if (!hadTouches) {
            if (world.gravity.y === 0.0) {
                ...
                addPipesAtLocations((Constants.MAX_WIDTH * 2) - (Constants.PIPE_WIDTH), world, entities)
                addPipesAtLocations((Constants.MAX_WIDTH * 3) - (Constants.PIPE_WIDTH), world, entities)
            }
            hadTouches = true
    ...
    Object.keys(entities).forEach(key => {
        if (key.indexOf("pipe") === 0 && entities.hasOwnProperty(key)) {
            Matter.Body.translate(entities[key].body, { x: -5.5, y: 0 })

            if (key.indexOf("Top") !== -1 && parseInt(key.replace("pipe", "")) % 2 === 0) {

                if (entities[key].body.position.x + 50 <= bird.position.x && !entities[key].scored) {
